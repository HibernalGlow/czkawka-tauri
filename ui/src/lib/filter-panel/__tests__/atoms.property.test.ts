...
describe('Filter Atoms - Property Tests', () => {
  ...
  const filterStateArb = fc.record({
    ...
    groupSize: fc.record({
      ...
      unit: fc.constantFrom('B', 'KB', 'MB', 'GB', 'TB') as fc.Arbitrary<'B' | 'KB' | 'MB' | 'GB' | 'TB'>,
    }),
    fileSize: fc.record({
      enabled: fc.boolean(),
      min: fc.integer({ min: 0, max: 1000000 }),
    ...
    extension: fc.record({
      ...
      mode: fc.constantFrom('include', 'exclude') as fc.Arbitrary<'include' | 'exclude'>,
    }),
    modifiedDate: fc.record({
      enabled: fc.boolean(),
      preset: fc.constantFrom('today', 'last7days', 'last30days', 'lastYear', 'custom') as fc.Arbitrary<'today' | 'last7days' | 'last30days' | 'lastYear' | 'custom'>,
      startDate: fc.option(fc.integer({ min: 0, max: Date.now() }), { nil: undefined }),
      endDate: fc.option(fc.integer({ min: 0, max: Date.now() }), { nil: undefined }),
    }),
    path: fc.record({
      enabled: fc.boolean(),
      mode: fc.constantFrom('contains', 'notContains', 'startsWith', 'endsWith') as fc.Arbitrary<'contains' | 'notContains' | 'startsWith' | 'endsWith'>,
    ...
    resolution: fc.record({
      enabled: fc.boolean(),
      minWidth: fc.option(fc.integer({ min: 0, max: 4000 }), { nil: undefined }),
      minHeight: fc.option(fc.integer({ min: 0, max: 4000 }), { nil: undefined }),
      maxWidth: fc.option(fc.integer({ min: 0, max: 8000 }), { nil: undefined }),
      maxHeight: fc.option(fc.integer({ min: 0, max: 8000 }), { nil: undefined }),
      aspectRatio: fc.constantFrom('16:9', '4:3', '1:1', 'any') as fc.Arbitrary<'16:9' | '4:3' | '1:1' | 'any'>,
    }),
    selectionOnly: fc.boolean(),
    showAllInFilteredGroups: fc.boolean(),
    preset: fc.constantFrom('none', 'largeFilesFirst', 'smallFilesFirst', 'recentlyModified', 'oldFiles') as fc.Arbitrary<'none' | 'largeFilesFirst' | 'smallFilesFirst' | 'recentlyModified' | 'oldFiles'>,
  });
      ...
      fc.assert(
        fc.property(
          ...
          (state) => {
            ...
            // 手动计算预期数量
            let expectedCount = 0;
            if (state.markStatus.enabled && state.markStatus.options.length > 0) expectedCount++;
            if (state.groupCount.enabled) expectedCount++;
            if (state.groupSize.enabled) expectedCount++;
            if (state.fileSize.enabled) expectedCount++;
            if (state.extension.enabled && state.extension.extensions.length > 0) expectedCount++;
