    ...
    it('should return items matching ANY selected status (OR logic)', () => {
      fc.assert(
        fc.property(
          ...
          (data, options) => {
            const selection = new Set(data.filter(() => Math.random() > 0.5).map(d => d.path));
          ...
          (data, minSize, maxSize) => {
            ...
            for (const groupId of resultGroupIds) {
              ...
              const totalSize = groupItems.reduce(
                (sum, item) => sum + (item.raw?.size ?? 0), 0
              );
          ...
          fc.array(
            fc.record({
              path: fc.constantFrom(
                '/test/file.jpg', '/test/file.png', '/test/file.txt',
                '/test/file.mp4', '/test/file.doc'
              ),
              groupId: fc.option(fc.integer({ min: 1, max: 5 }), { nil: undefined }),
            }),
            { minLength: 5, maxLength: 20 }
          ),
          fc.subarray(['jpg', 'png', 'txt', 'mp4', 'doc']),
          (data, extensions) => {
            const config = { enabled: true, extensions, mode: 'include' as const };
    ...
    it('should exclude files with matching extensions (exclude mode)', () => {
      fc.assert(
        fc.property(
          ...
          (data, extensions) => {
            const config = { enabled: true, extensions, mode: 'exclude' as const };
            const result = applyExtensionFilter(data, config);
      ...
      fc.assert(
        fc.property(
          ...
          (data, minSim, maxSim) => {
            ...
            for (const item of result) {
              const sim = Number.parseInt(item.similarity?.replace('%', '') || '100', 10);
