        ...
        fc.assert(
          fc.property(
            ...
            (allData, filteredData) => {
              ...
              const result = selectAllFiltered(filteredSubset, currentSelection);
      ...
      it('should preserve existing selections outside filtered data', () => {
        fc.assert(
          fc.property(
            ...
            (data) => {
              // 创建一些不在过滤数据中的选择
              const outsideSelection = new Set(['outside/path/1', 'outside/path/2']);
      ...
      fc.assert(
        fc.property(
          ...
          (data) => {
            ...
            // 返回数量应该等于过滤数据中被选中的数量
            const expectedCount = data.filter(d => selection.has(d.path)).length;
